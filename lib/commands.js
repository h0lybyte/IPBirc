...
Commands.prototype = {
  ...
  PASS: function(user, password) {
    ...
    var url = 'http://10.21.21.1:8080/api/tools/irc/irc.php';
    ...
    request.post(url, {
      ...
      if(!JSON.parse(JSON.stringify(raw)).status)
        {
  ...
  // Kick patch from TheBenji Fix: Don't crash on kick without user
